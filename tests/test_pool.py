...
def test_pool_object_representation():
    pool = Pool(1, 10)
    assert str(pool) == "<Pool [size:[1:10], free:0]>"


@pytest.mark.parametrize('minsize', [1.5, -1, -1.5])
def test_create_pool_object_validate_minsize(minsize):
    with pytest.raises(ValueError):
        Pool(minsize, 10)


@pytest.mark.parametrize('maxsize', [1.5, 0, -1, -1.5])
def test_create_pool_object_validate_maxsize(maxsize):
    with pytest.raises(ValueError):
        Pool(1, maxsize)


def test_create_pool_object_maxsize_should_be_greater_than_minsize():
    with pytest.raises(ValueError):
        Pool(1, 0)


# @pytest.mark.asyncio
# async def test_acquire(pool_factory):
...
@pytest.fixture
async def pool(mysql_params):
    pool = Pool(1, 10, **mysql_params)
    yield pool
    pool.close()
...
@pytest.mark.asyncio
async def test_acquire_as_context_manager(pool):
    assert pool.size == 0
...
@pytest.mark.asyncio
async def test_acquire_fails_when_pool_is_closed(pool):
    pool.close()
    assert pool.closed
    assert pool.size == 0
...
@pytest.mark.asyncio
async def test_release_as_context_manager(pool_factory):
    pool = await pool_factory(minsize=0)
    assert pool.freesize == 0
    async with pool.acquire() as conn:
        assert pool._used == {conn}
    assert pool.freesize == 1
    assert not pool._used


@pytest.mark.asyncio
@pytest.mark.parametrize('n', [1, 5, 10, 20, 100])
async def test_pool_01(n, pool):
    async def worker():
        async with pool.acquire() as conn:
...
# @pytest.mark.asyncio
# async def test_parallel_tasks(pool_creator):
...
# @pytest.mark.asyncio
# async def test_parallel_tasks_more(pool_creator):
...
# @pytest.mark.asyncio
# async def test__fill_free(pool_creator):
...
# @pytest.mark.asyncio
# async def test_true_parallel_tasks(pool_creator):
...
# @pytest.mark.asyncio
# async def test_wait_closed(pool_creator):
#     pool = await pool_creator(minsize=10, maxsize=10)
...
@pytest.mark.asyncio
async def test_release_closed_connection(pool):
    async with pool.acquire() as conn:
        conn.close()
        await conn.ensure_closed()
...
# @pytest.mark.asyncio
# async def test_close_with_acquired_connections(pool_creator):
#     pool = await pool_creator()
#     conn = await pool.acquire()
#     pool.close()
...
# @pytest.mark.asyncio
# async def test_drop_connection_if_timedout(pool_creator, connection_creator):
...
# @pytest.mark.asyncio
# async def test_cancelled_connection(pool_creator):
#     pool = await pool_creator(minsize=0, maxsize=1)
...
#             # timings)
#             task = asyncio.create_task(curs.execute(
...
# @pytest.mark.asyncio
# async def test_pool_with_connection_recycling(pool_creator):
...
# @pytest.mark.asyncio
# async def test_pool_drops_connection_with_exception(pool_creator):
